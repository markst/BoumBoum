...
internal class BoumBoumAnalyzer {
    ...
    init() {
        matrix = Array(repeating: (0,0), count: 11)
        values = Array(repeating: (-1, -1), count: 20)
        periods = Array(repeating: (-1, -1), count: 20)
        indexes = (0, 0, 0)
    }
    ...
    func pushValues(hsv: HSV) {
        if hsv.s > 0.5 && hsv.v > 0.5 {
            ...
            let filtered = processValue(hsv: hsv)
            if validFrameCount > minimumFrameCountToSettle {
                addNewValue(value: filtered, atTime: CGFloat(CACurrentMediaTime()))
            }
        }
        else {
            validFrameCount = 0
    ...
    private func addNewValue(value: CGFloat, atTime time: CGFloat) {
        ...
        let averageUpValue = upFiltered.reduce(0, +) / CGFloat(upFiltered.count)
        let downFiltered = values.flatMap { e in
            return e.down != -1 ? e.down : nil
        }
        let averageDownValue = downFiltered.reduce(0, +) / CGFloat(downFiltered.count)
    ...
    func averageBoumBoum() -> UInt? {
        ...
        if filtered.count > 2 {
            let average = filtered.reduce(0, { $0 + $1.val }) / CGFloat(filtered.count)
...
extension Array {
    internal func shifted(delta: Int = 1) -> [Element] {
        var new = [Element]()
        for e in enumerated() {
            new.append(self[(e.offset + delta) % count])
        }
        return new
    }
}
